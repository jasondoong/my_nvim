...
function M.run_nearest()
  ...
  if not ok_parser or not parser then
    vim.notify('Tree-sitter parser unavailable for python.', vim.log.levels.ERROR)
    return
  end
  parser:parse()
  ...
  elseif vim.treesitter.get_node_at_cursor then
    node = vim.treesitter.get_node_at_cursor()
  end
  if not node then
    vim.notify('Tree-sitter node not available at cursor.', vim.log.levels.ERROR)
    return
  end
